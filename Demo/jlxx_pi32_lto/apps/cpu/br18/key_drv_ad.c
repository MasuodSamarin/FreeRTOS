...
static void __adc_roundrobin(void)
{
    ...
    /* printf("ADC ch %d res %x \n", channel,  __this->value[channel]); */
...
static void ad_key_init(void)
{
    ...
    __this->timer = periodic_timer_add(20, __adc_roundrobin);
}
...
static void ad_key_ctrl(u32 ctrl, ...)
{
    va_list argptr;
