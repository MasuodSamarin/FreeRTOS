...
u32 get_lsb_clk_freq(void)
{
    return g_clk_info.lsb_clk;
}

u32 get_hsb_clk_freq(void)
{
    return g_clk_info.hsb_clk;
}

u32 get_sfc_clk_freq(void)
{
...
{
    log_info("[Clock Dump]\n");
    /* log_info("Internal OSC CLK : %d\n", __this->iosc_clk); */
    /* log_info("OSC CLK : %d\n",        __this->osc_clk); */

    /* log_info("PLL SYS CLK : %d\n",    g_clk_info.hsb_clk); */
    /* log_info("PLL APC CLK : %d\n",    __this->apc_clk); */
    /* log_info("PLL ALNK CLK : %d\n",   __this->alnk_clk); */
