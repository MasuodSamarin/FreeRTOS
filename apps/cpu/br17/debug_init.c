...
static void sys_tick_1s(void)
{
    printf("os tick %d\n", rtos_time_get());

    /* puts("send\n"); */
}
static void app_sys_tick_1s(struct sys_timer *timer)
{
    sys_tick_1s();
    sys_timer_register(&__this->timeout_1s, 1000, app_sys_tick_1s);
}
...
const char pubDate[] = "\r\n*********Btstack LE Main "__DATE__ " "__TIME__ "******\r\n";
...
static void app_debug_init(void)
{
    ...
    /* __this->timer[0] = periodic_timer_add(500, sys_tick_500ms); */
