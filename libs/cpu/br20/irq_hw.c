...
static void __hw_irq_enable(u32 index)
{
    ...
    if (index < 32) {
        ...
        __asm__ volatile("mov ie0,%0\n\tj 0" :: "r"(tmp));
    } else {
        index = index - 32;
        __asm__ volatile("mov %0,ie1" : "=r"(tmp));
        tmp |= BIT(index);
        __asm__ volatile("mov ie1,%0\n\tj 0" :: "r"(tmp));
    }
}

AT_IRQ_HW_CODE
static bool __hw_irq_staus(u32 index)
{
    int tmp;
...
static void hw_irq_handler_unregister(unsigned int irq_idx)
{
    ...
    israddr[irq_idx] = (unsigned int)error_isr_handler;
