...
static void __set_sample_rate(u16 sr)
{
    ...
    SFR(JL_AUDIO->DAC_CON, 0, 4, index);
}

static void __set_digital_enable(void)
{
    SFR(JL_AUDIO->DAC_CON, 4, 1, 1);
}

static void __set_digital_disable(void)
{
    SFR(JL_AUDIO->DAC_CON, 4, 1, 0);
}

static void __set_digital_ie(void)
{
    SFR(JL_AUDIO->DAC_CON, 5, 1, 0);
}

static void __set_dc_cancelling_fileter(u8 value)
{
    SFR(JL_AUDIO->DAC_CON, 12, 4, value);
}

static void __set_dma_address(u8 *addr)
{
    ASSERT(((int)addr % 3) == 0, "%s\n", "DMA not algin!");

    JL_AUDIO->DAC_ADR = (volatile unsigned int)addr;
}
...
{
    SFR(JL_AUDIO->DAA_CON0, 0, 1, 1);
}

static void __set_analog_disable(void)
{
    SFR(JL_AUDIO->DAA_CON0, 0, 1, 0);
}

static void __set_current_sel(u8 type)
{
    /* SFR(JL_AUDIO->DAA_CON4, 0, 3, type); */
}

static void __set_mute_enable(void)
{
    SFR(JL_AUDIO->DAA_CON0, 7, 1, 1);
}
...
{
    SFR(JL_AUDIO->DAA_CON0, 4, 1, l_en);
    SFR(JL_AUDIO->DAA_CON0, 5, 1, r_en);
}

static void __set_output_channel_gain(u8 l_gain, u8 r_gain)
{
    SFR(JL_AUDIO->DAA_CON1, 0, 5, l_gain);
    SFR(JL_AUDIO->DAA_CON1, 8, 5, l_gain);
}
...
static void __dac_out_process(void)
{
    ...
    buf_idx = JL_AUDIO->DAC_CON & BIT(8);
    buf_idx >>= 8;
